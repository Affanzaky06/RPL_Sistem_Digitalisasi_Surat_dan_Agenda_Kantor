<x-layout :role="$role">
                                    ...
                                    <td class="py-3">
                                        ...
                                        @if(in_array($item->status, ['Tidak Hadir', 'Ditolak']))
                                            <div class="d-flex flex-column gap-1 align-items-center">
                                                <a href="javascript:void(0)" class="text-primary text-decoration-none fw-medium" data-bs-toggle="modal" data-bs-target="#dispoUlangModal{{ $item->id_disposisi }}">DispoUlang</a>
                                                
                                                <form action="{{ route('laporan.setujui', $item->id_disposisi) }}" method="POST">
                                                    @csrf
                                                    <button type="submit" class="btn btn-link text-success text-decoration-none p-0 fw-medium" style="font-size: 0.85rem;" onclick="return confirm('Maklumi penolakan ini tanpa memanggil orang lain?')">Setujui</button>
                                                </form>
                                            </div>
                                        @else
    ...
    @foreach ($laporan as $item)
                    ...
                    <div class="modal-footer">
                        <div class="d-flex justify-content-between w-100">
                            @if (in_array($item->status, ['Menunggu Konfirmasi', 'Belum Dibaca']))
                                ...
                                @endphp
                                <form action="{{ route($rolePrefix.'.disposisi.batal', $item->id_disposisi) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn btn-danger" onclick="return confirm('Yakin ingin membatalkan disposisi ini?')">
        ...
        <!-- MODAL DISPOSISI ULANG (KHUSUS JIKA DITOLAK/TIDAK HADIR) -->
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content border-0 shadow rounded-4 p-2">
                    <div class="modal-header border-0 pb-0">
                        <h5 class="fw-bold fs-4">Disposisi Ulang Surat</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>

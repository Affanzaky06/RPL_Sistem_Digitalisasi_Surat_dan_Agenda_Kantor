    ...
    if (calendarEl) {
        ...
        let calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventContent: function (arg) {
                ...
                let start = event.start.toLocaleTimeString("id-ID", {
                    hour: "2-digit",
                    minute: "2-digit",
                });
                let end = event.end
                ...
                if (dynamicStatus === "terlaksana") {
                    bgClass = "bg-terlaksana";
                    badgeClass = "badge-terlaksana";
                    badgeText = "Terlaksana";
                } else if (dynamicStatus === "berlangsung") {
                    bgClass = "bg-berlangsung";
                    badgeClass = "badge-berlangsung";
                    badgeText = "Sedang Berlangsung";
                } else {
                    bgClass = "bg-mendatang";
                    badgeClass = "badge-mendatang";
            ...
            eventClick: function (info) {
                ...
                if (currentView === "timeGridDay") {
                    ...
                    let calBoxRect = calendarEl
                        .closest(".border")
                        .getBoundingClientRect();
                    left =
                        calBoxRect.width / 2 -
                        popupWidth / 2 +
                        (calBoxRect.left - containerRect.left);
        ...
        function showModalTolakAlasan(idAgenda, eventProps) {
            ...
            // Isi data modal
            document.getElementById("modal-pengirim").textContent =
                eventProps.pengirim || "-";
            document.getElementById("modal-nomor-surat").textContent =
                eventProps.nomor_surat || "-";
            document.getElementById("modal-perihal").textContent =
                eventProps.perihal || "-";
            document.getElementById("modal-tanggal-surat").textContent =
                eventProps.tanggal_surat || "-";
            document.getElementById("modal-waktu").textContent =
                eventProps.waktu || "-";
            ...
            modalForm.querySelector(
                'textarea[name="alasan_tidak_hadir"]',
            ).value = "";

            let modal = new bootstrap.Modal(
                document.getElementById("tidakHadirModal"),
            );
            modal.show();
        }
        ...
        function showBatalModalWithDispo(idAgenda, data, eventProps) {
            ...
            if (data.ada_pendamping) {
                ...
                let listContainer = document.getElementById(
                    "list-pendamping-konfirmasi",
                );
                let html =
                    '<hr><p class="mb-2 text-muted small">Pilih pendamping yang akan menjadi perwakilan:</p>';
                ...
                let modalKonfirmasi = new bootstrap.Modal(
                    document.getElementById("modalKonfirmasiPendamping"),
                );

                // Tombol "Disposisikan ke Pendamping"
                let btnShowPendamping = document.getElementById(
                    "btn-show-pendamping",
                );
                let newBtnShow = btnShowPendamping.cloneNode(true);
                btnShowPendamping.parentNode.replaceChild(
                    newBtnShow,
                    btnShowPendamping,
                );
                ...
                let btnBatalkanSemua = document.getElementById(
                    "btn-batalkan-semua-agenda",
                );
                let newBtnBatalSemua = btnBatalkanSemua.cloneNode(true);
                btnBatalkanSemua.parentNode.replaceChild(
                    newBtnBatalSemua,
                    btnBatalkanSemua,
                );
                ...
                let fallbackButtons =
                    document.getElementById("fallback-buttons");
                let btnTolakKirimAlasan = document.getElementById(
                    "btn-tolak-kirim-alasan",
                );
                if (btnTolakKirimAlasan && fallbackButtons) {
                    if (userRole === "Kepala") {
                        ...
                        btnTolakKirimAlasan.parentNode.replaceChild(
                            newBtnTolak,
                            btnTolakKirimAlasan,
                        );
                ...
                if (userRole === "Kepala") {
                    ...
                    let modalPilih = new bootstrap.Modal(
                        document.getElementById("modalPilihAksiBatal"),
                    );

                    let btnDisposisi =
                        document.getElementById("btn-aksi-disposisi");
                    let newBtnDisposisi = btnDisposisi.cloneNode(true);
                    btnDisposisi.parentNode.replaceChild(
                        newBtnDisposisi,
                        btnDisposisi,
                    );
        ...
        function showModalDisposisiBatal(idAgenda, data, eventProps) {
            ...
            if (eventProps) {
                document.getElementById("dispo-batal-pengirim").textContent =
                    eventProps.pengirim || "-";
                document.getElementById("dispo-batal-nomor").textContent =
                    eventProps.nomor_surat || "-";
                document.getElementById("dispo-batal-perihal").textContent =
                    eventProps.perihal || "-";
                document.getElementById("dispo-batal-tanggal").textContent =
                    eventProps.tanggal_surat || "-";
                document.getElementById("dispo-batal-jenis").textContent =
                    eventProps.jenis_surat || "-";

                let prioritasEl = document.getElementById(
                    "dispo-batal-prioritas",
                );
            ...
            let modalDispo = new bootstrap.Modal(
                document.getElementById("modalDisposisiBatal"),
            );
            modalDispo.show();
        }

        // SEARCH & FILTER UNTUK MODAL DISPOSISI BATAL
        const searchInputBatal = document.querySelector(
            ".search-penerima-batal",
        );
        const filterSelectBatal = document.querySelector(
            ".filter-jabatan-penerima-batal",
        );
        const listContainerBatal = document.getElementById(
            "list-penerima-batal",
        );

        function filterListBatal() {
            if (!searchInputBatal || !filterSelectBatal || !listContainerBatal)
                return;
            const keyword = searchInputBatal.value.toLowerCase().trim();
            const jabatan = filterSelectBatal.value;

            listContainerBatal
                .querySelectorAll(".penerima-item-batal")
                .forEach((item) => {
        ...
        if (searchInputBatal)
            searchInputBatal.addEventListener("input", filterListBatal);
        if (filterSelectBatal)
            filterSelectBatal.addEventListener("change", filterListBatal);
